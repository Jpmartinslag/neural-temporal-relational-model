#!/bin/bash
# HERALD -- submit France ZE2020 dynamic graph falsification HPC.
...
OUTDIR="hpc_results/fr_ze2020_dynamic_graph_falsifications_${RUN_ID}"
LOGDIR="${OUTDIR}/logs"
EDGES_PATH="${FR_ZE2020_DYNAMIC_GRAPH_EDGES:-data/processed/france_ze2020/fr_ze2020_dynamic_graph_edges.csv}"

echo "=========================================="
echo "France ZE2020 dynamic graph falsifications submit -- RUN_ID=${RUN_ID}"
echo "EDGES_PATH=${EDGES_PATH}"
echo "=========================================="

if [[ ! -f "src/modeles/france_ze2020/run_fr_ze2020_dynamic_graph_falsifications.py" ]]; then
  echo "ERROR: dynamic graph falsification script not found -- did you rsync first?" >&2
  exit 1
fi
...
SBATCH_CMD=(sbatch
  ...
  "hpc/france_ze2020_dynamic_graph/run_fr_ze2020_dynamic_graph_falsification_array.sbatch"
)
