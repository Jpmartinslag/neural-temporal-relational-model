#!/bin/bash
# Run one HERALD Phase 4B seed for one country (all configs sequentially).
# Phase 4B: extended AR features (side5_full) + lag2 tensor ablation.
#
# Required env vars: SEED, COUNTRY, OUT_ROOT
# Optional: EPOCHS (default 800), MASK_WARMUP (default 100), DEVICE, PYTHON
...
check_inputs() {
  ...
  # Verify side_lag_2 exists in panel (Phase 4B requirement)
  "$PYTHON" hpc/phase4/check_phase4b_panel.py "${PHASE4_BASE}/panel_ze2020.csv" "$COUNTRY"
}
...
run_config() {
  ...
  local tag="phase4b_${COUNTRY}_${label}"
...
echo "============================================================"
echo " HERALD Phase 4B — ${COUNTRY^^} seed battery"
...
source "$(dirname "$0")/phase4b_configs.sh"
