#!/bin/bash
# Submit Phase 4E-C EU macro-signal ablation for all 4 countries.
# 10 seeds × 6 configs × 4 countries = 240 runs.
...
echo "=== Phase 4E-C submit $(date '+%Y-%m-%d %H:%M:%S') ==="
echo "Seeds : $SEEDS"
echo "Epochs: $EPOCHS"
...
for COUNTRY in fr nl be pt; do
  n_configs=$(COUNTRY="$COUNTRY" bash -lc 'source hpc/phase4/phase4e_c_configs.sh && phase4e_c_configs | wc -l')
  OUT_ROOT="hpc_results/herald_phase4e_c_${COUNTRY}_${TIMESTAMP}_r1"
  if [ -d "$OUT_ROOT" ]; then
  ...
  JOB_ID=$(sbatch \
    ...
    hpc/phase4/run_herald_phase4e_c_array.sbatch \
    | awk '{print $NF}')
...
echo "  python3 hpc/phase4/audit_phase4e_c_results.py \\"
echo "    --root-fr hpc_results/herald_phase4e_c_fr_${TIMESTAMP}_r1 \\"
echo "    --root-nl hpc_results/herald_phase4e_c_nl_${TIMESTAMP}_r1 \\"
echo "    --root-be hpc_results/herald_phase4e_c_be_${TIMESTAMP}_r1 \\"
echo "    --root-pt hpc_results/herald_phase4e_c_pt_${TIMESTAMP}_r1"
