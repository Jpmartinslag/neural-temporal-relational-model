#!/bin/bash
# Run one HERALD Phase 4E-C seed for one country.
# EU macro-signal ablation on top of Phase 4E-B per-country winners.
...
run_config() {
  ...
  local qtensor_col="$6"
  local macro_feature_set="$7"

  local run_tag="phase4e_c_${COUNTRY}_${label}"
  ...
  echo ">> [${COUNTRY}][seed=${SEED}] label=${label} wrapper=${wrapper_type} features=${feature_policy} qtensor=${qtensor_policy} macro=${macro_feature_set}"
...
source hpc/phase4/phase4e_c_configs.sh
...
while IFS= read -r line; do
  ...
  macro_feature_set=$(echo "$line"| awk '{print $7}')
  run_config "$label" "$wrapper_type" "$feature_policy" "$qtensor_policy" "$qtensor_path" "$qtensor_col" "$macro_feature_set"
done < <(phase4e_c_configs)
