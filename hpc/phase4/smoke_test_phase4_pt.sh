#!/bin/bash
# CPU smoke test — HERALD Phase 4 Portugal.
# 1 seed × 1 epoch × 3 configs = 3 JSON artifacts expected.
# ⚠️  tensor = sector_births_tensor (proxy) — NOT Q7 effectifs
set -euo pipefail
...
OUT_SMOKE=${OUT_SMOKE:-"hpc_results/herald_phase4_pt_smoke_${STAMP}"}
...
bash -n hpc/phase4/submit_herald_phase4_pt.sh
...
echo "PT tensor guard OK — runtime labels are checked after smoke artifacts are written"

"$PYTHON" hpc/phase4/prepare_phase4_panel.py --country pt
...
root = Path("${OUT_SMOKE}")
expected_labels = {"baseline_side2", "sector_births_lag1", "no_qtensor_control"}
found_labels = set()
...
for path in sorted((root / "reports/per_run").glob("*.json")):
    ...
    qtensor_policy = result.get("quarterly_tensor_policy", "?")
    print(f"  {tag}: wmape={wmape} qtensor_policy={qtensor_policy}")

    # Q7 guard — tensor policy must not be effectifs in the PT context
    # (effectifs_lag1 is the underlying policy used but the config label must be sector_births_lag1)
    for label in expected_labels:
        if label in tag:
            found_labels.add(label)

    # Verify no qtensor_jobs label appears in PT output
    assert "qtensor_jobs" not in tag, f"Q7 GUARD: found qtensor_jobs in PT tag: {tag}"
...
print("Q7 guard: no qtensor_jobs label in PT output OK")
PY

echo ""
echo "============================================================"
echo " Phase 4 PT smoke OK"
echo " artifacts: $OUT_SMOKE"
echo " configs:   baseline_side2 / sector_births_lag1 / no_qtensor_control"
echo " ⚠️  sector_births_lag1 is a PROXY tensor (births, not employment)"
echo "============================================================"
