#!/bin/bash
# Run one HERALD Phase 4E-A2 seed for one country.
...
run_config() {
  ...
  local qtensor_path="$4"
  local qtensor_col="$5"

  local run_tag="phase4e_a2_${COUNTRY}_${label}"
  ...
  echo ">> [${COUNTRY}][seed=${SEED}] label=${label} features=${feature_policy} qtensor=${qtensor_policy}"
  ...
  "$PYTHON" hpc/phase4/run_herald_phase4e_a2_wrapper.py \
...
source hpc/phase4/phase4e_a2_configs.sh
...
while IFS= read -r line; do
  ...
  qtensor_path=$(echo "$line" | awk '{print $4}')
  qtensor_col=$(echo "$line" | awk '{print $5}')
  run_config "$label" "$feature_policy" "$qtensor_policy" "$qtensor_path" "$qtensor_col"
done < <(phase4e_a2_configs)
