#!/bin/bash
# Submit HERALD Phase 3C URSSAF isolation battery.
#
# 4 configs × 10 seeds = 40 runs
# C0_no_urssaf  — baseline with q_tensor zeroed
# C3_no_urssaf  — urssaf_employer_estab_growth + no q_tensor
# C4_no_urssaf  — permuted tutor + no q_tensor
# C10_no_urssaf — spatial_perm tutor + no q_tensor
#
# Usage:
#   bash hpc/regime/submit_herald_phase3c_urssaf_isolation.sh
#
# OUT_ROOT: hpc_results/herald_regime_phase3c_urssaf_isolation_<STAMP>_r1
set -euo pipefail

STAMP=${STAMP:-$(date +%Y%m%d_%H%M%S)}
REGIME_PLAN=phase3c_urssaf_isolation
OUT_ROOT=${OUT_ROOT:-"hpc_results/herald_regime_phase3c_urssaf_isolation_${STAMP}_r1"}
...
echo ""
echo "============================================================"
echo " Phase 3C URSSAF Isolation — Submit"
