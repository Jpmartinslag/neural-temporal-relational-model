#!/bin/bash
# HERALD Phase 2H macro-feature battery — safe preflight + submit.
set -euo pipefail

STAMP=${STAMP:-$(date +%Y%m%d_%H%M%S)}
REGIME_PLAN=${REGIME_PLAN:-phase2h_macro_real}
OUT_ROOT=${OUT_ROOT:-"hpc_results/herald_regime_${REGIME_PLAN}_${STAMP}_r1"}
...
case "$REGIME_PLAN" in
  phase2h_macro_real) EXPECTED=100 ;;
  phase2h_macro_permute) EXPECTED=40 ;;
  phase2h_macro_extra) EXPECTED=40 ;;
  *) echo "ERROR: REGIME_PLAN must be phase2h_macro_real, phase2h_macro_permute, or phase2h_macro_extra" >&2; exit 1 ;;
esac

echo "========================================================"
echo " HERALD Phase 2H — Macro Preflight + Submit"
...
bad = panel[required].isna().mean().to_dict()
print("macro_na_share", bad)
print("panel_shape", panel.shape)
PY
