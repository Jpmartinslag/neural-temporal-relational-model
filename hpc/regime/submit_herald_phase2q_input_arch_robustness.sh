#!/bin/bash
# HERALD Phase 2Q input policy × architecture robustness — safe preflight + submit.
set -euo pipefail

STAMP=${STAMP:-$(date +%Y%m%d_%H%M%S)}
REGIME_PLAN=phase2q_input_arch_robustness
OUT_ROOT=${OUT_ROOT:-"hpc_results/herald_regime_phase2q_input_arch_robustness_${STAMP}_r1"}
