...
| 9 | Relational-learner gradient/mechanics tests | PASS | `test_h9` (no self-loops), `test_h12` (no node-only path), `test_h13` (top-k does not block gradient), `test_h17`/`test_h18` (the proposed model, NRI, and MTGNN share one candidate set) |
