...
`src/modeles/synthetic/` and `src/modeles/real_world/` research track (relation-learning
experiments behind `HERALD_93`–`HERALD_98`), not for the minimal example or the fast test
suites below. Install it separately (`pip install torch`) if you need that track.
...
Rolling-origin evaluation and the synthetic benchmark use independent runs across 5 seeds
(`HERALD_93`) or per-seed reruns documented in each `HERALD_9X` report; seeds are fixed and
recorded in the corresponding job's artefact JSON under `hpc_results/`, not derived from wall-clock
time or randomness at run time.

## HPC vs. local

- **Local / this repository**: baselines, the fast test suites above, and any script listed in
  `reports/canonical/HERALD_10_CODE_PATH_MAP.md` under `ACTIVE_*` without an `hpc/` counterpart.
- **HPC (SLURM, Mésocentre)**: the dynamic-graph training, relation-objective, and the HERALD
  93-98 synthetic benchmark runs. Submission scripts live under `hpc/<phase>/`; **none of them
  were (re)launched by this cleanup** — the committed `hpc_results/` artefacts are the frozen
  output of runs already completed. See `reports/canonical/HERALD_11_HPC_AND_RESULTS_MAP.md` for
  which job produced which result.
