"""
HERALD Semi — irmão do V6 com mascaramento espacial no treino

Derivado de train_herald_v6.py. A única diferença arquitectural é o
mascaramento estocástico de zonas durante o treino (--mask-ratio).

Mecanismo:
  A cada epoch (após --mask-warmup-epochs de aquecimento), cada zona é
  mascarada independentemente com probabilidade mask_ratio. Features
  anuais e trimestrais da zona mascarada são zeradas naquele timestep.
  O modelo é forçado a recuperar a predição da zona usando vizinhos via
  A_t. A loss é computada normalmente para todas as zonas — incluindo
  as mascaradas. Isso induz o grafo a descobrir dependências territoriais
  latentes que vão além dos priors de mobilidade e geografia.

Objetivo científico:
  - Descobrir co-movimentos econômicos não capturados por mobilidade/geo
  - Comparar A_t(semi) vs A_t(v6) para identificar relações latentes
  - Validar estabilidade: re-rodar com splits pré-COVID (2012-2019)
    para separar correlações estruturais de artefatos do choque

Saídas completamente isoladas do V6 (prefixo herald_semi_):
  data/processed/herald_semi_predictions_total_{ablation}{tag}_seed_{seed}_v1.csv
  data/processed/herald_semi_predictions_sector_{ablation}{tag}_seed_{seed}_v1.csv
  data/processed/herald_semi_internals_{ablation}{tag}_seed_{seed}_v1.npz
  reports/herald_semi_metrics_v1.json
  reports/HERALD_SEMI_MODEL_V1.md
"""
...
OUT_JSON = REPORTS / "herald_semi_metrics_v1.json"
OUT_MD   = REPORTS / "HERALD_SEMI_MODEL_V1.md"
...
def train_herald_v6(seq, adj_geo, adj_mob, args, device):
    ...
    mask_warmup   = getattr(args, "mask_warmup_epochs", 50)
    mask_strategy = getattr(args, "mask_strategy", "random")
    semi_lambda   = getattr(args, "semi_lambda", 0.0)
    semi_target   = getattr(args, "semi_target", "total")

    model.train()
    for ep in range(args.epochs):
        opt.zero_grad()

        # Spatial masking — three strategies:
        #   random: each zone masked independently per timestep (soft, diverse)
        #   block:  same zones masked across ALL timesteps in this epoch (hard,
        #           forces the graph to fully replace a zone's own signal)
        #   spatial_block: seed a dropped zone and expand through its strongest
        #           mobility/geographic neighbours, testing territorial recovery.
        drop_mask = torch.zeros(T, N, dtype=torch.bool, device=device)
        if mask_ratio > 0 and ep >= mask_warmup:
            if mask_strategy == "block":
                # (N,) mask reused for every timestep
                block_drop = torch.rand(N, device=device) < mask_ratio
                drop_mask = block_drop.unsqueeze(0).expand(T, N)
                keep = (~block_drop).float()
                ann_list = [x_ann[t] * keep.unsqueeze(-1)                       for t in range(T)]
                q_list   = [x_q[t].permute(1, 0, 2) * keep.unsqueeze(-1).unsqueeze(-1) for t in range(T)]
            elif mask_strategy == "spatial_block":
                target_k = max(1, int(round(mask_ratio * N)))
                prior = adj_m_t + adj_g
                prior = prior - torch.diag_embed(torch.diagonal(prior))
                seed_idx = int(torch.randint(0, N, (1,), device=device).item())
                scores = prior[seed_idx]
                if target_k > 1:
                    neigh = torch.topk(scores, k=min(target_k - 1, N - 1)).indices
                    block_idx = torch.cat([
                        torch.tensor([seed_idx], device=device, dtype=torch.long),
                        neigh,
                    ])
                else:
                    block_idx = torch.tensor([seed_idx], device=device, dtype=torch.long)
                block_drop = torch.zeros(N, dtype=torch.bool, device=device)
                block_drop[block_idx] = True
                drop_mask = block_drop.unsqueeze(0).expand(T, N)
                keep = (~block_drop).float()
                ann_list = [x_ann[t] * keep.unsqueeze(-1)                       for t in range(T)]
                q_list   = [x_q[t].permute(1, 0, 2) * keep.unsqueeze(-1).unsqueeze(-1) for t in range(T)]
            else:
                # random: independent (T, N) mask
                spatial_drop = torch.rand(T, N, device=device) < mask_ratio
                drop_mask = spatial_drop
                ann_list = [x_ann[t] * (~spatial_drop[t]).float().unsqueeze(-1)                       for t in range(T)]
                q_list   = [x_q[t].permute(1, 0, 2) * (~spatial_drop[t]).float().unsqueeze(-1).unsqueeze(-1) for t in range(T)]
        else:
        ...
        main_raw  = huber(pred_main, target)
        loss_main = (main_raw * mask * zone_w_bc).sum() / denom
        ...
        if semi_lambda > 0.0 and mask_ratio > 0.0 and ep >= mask_warmup:
            masked_main = drop_mask.float() * mask
            if semi_target in ("total", "total_a10"):
                denom_m = torch.clamp((masked_main * zone_w_bc).sum(), min=1.0)
                loss_semi = loss_semi + (main_raw * masked_main * zone_w_bc).sum() / denom_m
            if semi_target in ("a10", "total_a10"):
                masked_sec = drop_mask.float() * sec_m
                denom_s = torch.clamp(masked_sec.sum(), min=1.0)
                loss_semi = loss_semi + (kl.sum(-1) * masked_sec).sum() / denom_s
        ...
        loss = loss_main + lam_sec * loss_sec + semi_lambda * loss_semi + loss_graph
        loss.backward()
...
def main():
    ...
    parser.add_argument("--mask-ratio", type=float, default=0.1,
                        help="Fraction of zones masked per timestep during training (0 = disabled).")
    parser.add_argument("--mask-warmup-epochs", type=int, default=100,
                        help="Epochs before spatial masking activates.")
    parser.add_argument("--mask-strategy", default="random",
                        choices=["random", "block", "spatial_block"],
                        help="random: independent per-timestep mask; "
                             "block: same zones masked for the entire epoch; "
                             "spatial_block: contiguous block around a graph-neighbourhood seed.")
    parser.add_argument("--semi-lambda", type=float, default=0.0,
                        help="Extra weight on masked-node reconstruction loss (0 = masking augmentation only).")
    parser.add_argument("--semi-target", default="total",
                        choices=["total", "a10", "total_a10"],
                        help="Auxiliary masked reconstruction target used by --semi-lambda.")
    ...
    if not args.forecast_only:
        ...
        out_total  = args.prediction_output_dir / f"herald_semi_predictions_total_{suffix}_v1.csv"
        out_sector = args.prediction_output_dir / f"herald_semi_predictions_sector_{suffix}_v1.csv"
        out_int    = args.prediction_output_dir / f"herald_semi_internals_{suffix}_v1.npz"
        ...
        np.savez_compressed(
            ...
            mask_ratio            = np.array([args.mask_ratio]),
            mask_warmup_epochs    = np.array([args.mask_warmup_epochs]),
            mask_strategy         = np.array([args.mask_strategy]),
            semi_lambda           = np.array([args.semi_lambda]),
            semi_target           = np.array([args.semi_target]),
        )
    ...
    if args.forecast_horizon > 0:
        ...
        out_forecast = args.prediction_output_dir / f"herald_semi_forecast_{suffix}_v1.csv"
