...
METRICS_OUT = ROOT / "reports" / "segmented_hub_shrinkage_baseline_metrics_v0.json"
REPORT_OUT = ROOT / "reports" / "SEGMENTED_HUB_SHRINKAGE_BASELINE_V0.md"
...
def build_payload(data):
    ...
    for target_year in EVALUATION_TARGET_YEARS:
        ...
        declining_hub_mask = hub_mask & np.isfinite(growth_vs_lag) & (growth_vs_lag <= NEG_GROWTH_EPS)
        ...
        payload[int(target_year)] = {
            ...
            "lag": lag_test_valid,
            "hub_mask": hub_mask,
            "declining_hub_mask": declining_hub_mask,
...
def apply_gamma(payload, gamma):
    ...
    mask = payload["declining_hub_mask"]
...
def write_report(payload):
    ...
    candidate = payload["summary"]["segmented_hub_shrinkage_baseline"]
    cmp = payload["comparison_vs_ridge_lag_nbcom"]
    lines = [
        "# Segmented Hub Shrinkage Baseline v0",
        "",
        "Date : 2026-04-21",
        "",
        "Shrinkage appliqué uniquement aux hubs en décroissance observée sur l'année cible.",
        "",
        f"- ridge_lag_nbcom: `{baseline['mean_wmape']:.3f}`",
        f"- segmented_hub_shrinkage_baseline: `{candidate['mean_wmape']:.3f}`",
...
def main():
    ...
    for item in gamma_trace:
        ...
        pred = apply_gamma(payload[year], item["selected_gamma"])
        rows.append(
            {
                "target_year": int(year),
                "model": "segmented_hub_shrinkage_baseline",
                "wmape": wmape(payload[year]["y_true"], pred),
                "selected_gamma": float(item["selected_gamma"]),
                "declining_hub_share": float(np.mean(payload[year]["declining_hub_mask"])),
    ...
    result = {
        ...
        "rows": rows,
        "summary": summary,
        "comparison_vs_ridge_lag_nbcom": compare(summary["segmented_hub_shrinkage_baseline"], summary["ridge_lag_nbcom"]),
    }
