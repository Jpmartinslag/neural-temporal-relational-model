...
METRICS_OUT = ROOT / "reports" / "temporal_mlp_with_rei_metrics_v0.json"
REPORT_OUT = ROOT / "reports" / "TEMPORAL_MLP_WITH_REI_V0.md"
PRED_OUT = ROOT / "data" / "processed" / "temporal_mlp_with_rei_predictions_v0.csv"
    ...
    for target_year in TARGET_YEARS:
        ...
        pred_rows.append(
            ...
            .assign(
                ...
                pred_temporal_mlp_with_rei=pred_mlp,
            )
        )
        ...
        summary_rows.append(
            {
                ...
                "temporal_mlp_with_rei_wmape": wmape(y_test, pred_mlp),
            }
        )
...
def main():
    ...
    mlp_mean = float(np.mean([r["temporal_mlp_with_rei_wmape"] for r in rows]))
    payload = {
        ...
        "comparison_vs_rei_created_baseline": {
            ...
            "per_year_delta": {
                str(r["target_year"]): float(r["temporal_mlp_with_rei_wmape"] - r["rei_created_baseline_wmape"]) for r in rows
            },
            "worsened_years": [
                int(r["target_year"]) for r in rows if r["temporal_mlp_with_rei_wmape"] - r["rei_created_baseline_wmape"] > 1e-6
            ],
            "strictly_better_with_tolerance": bool(
                mlp_mean < rei_mean and all(r["temporal_mlp_with_rei_wmape"] - r["rei_created_baseline_wmape"] <= 1e-6 for r in rows)
            ),
        },
