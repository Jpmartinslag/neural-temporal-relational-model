...
METRICS_OUT = ROOT / "reports" / "residual_gcn_with_rei_metrics_v0.json"
REPORT_OUT = ROOT / "reports" / "RESIDUAL_GCN_WITH_REI_V0.md"
PRED_OUT = ROOT / "data" / "processed" / "residual_gcn_with_rei_predictions_v0.csv"
...
def evaluate(df, adj):
    ...
    for target_year in TARGET_YEARS:
        ...
        train_pivot_x = (
            train_df.pivot(index="target_year", columns="node_idx", values=kept_features)
            .sort_index(axis=0)
            .sort_index(axis=1)
        )
        test_pivot_x = (
            test_df.pivot(index="target_year", columns="node_idx", values=kept_features)
            .sort_index(axis=0)
            .sort_index(axis=1)
        )
        train_pivot_r = (
            train_df.pivot(index="target_year", columns="node_idx", values="residual_target")
            .sort_index(axis=0)
            .sort_index(axis=1)
        )
...
def main():
    ...
    payload = {
        "model": "residual_gcn_with_rei_geo",
