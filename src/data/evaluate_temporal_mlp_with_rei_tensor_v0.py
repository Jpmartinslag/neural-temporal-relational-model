...
METRICS_OUT = ROOT / "reports" / "temporal_mlp_with_rei_tensor_metrics_v0.json"
REPORT_OUT = ROOT / "reports" / "TEMPORAL_MLP_WITH_REI_TENSOR_V0.md"
PRED_OUT = ROOT / "data" / "processed" / "temporal_mlp_with_rei_tensor_predictions_v0.csv"
...
def load_frame():
    ...
    feature_idx = [feature_names.index(name) for name in FEATURES]
    ...
    for pos, year in enumerate(package["years"].astype(int)):
        ...
        for feat, idx in zip(FEATURES, feature_idx):
...
def evaluate(df):
    ...
    for target_year in TARGET_YEARS:
        ...
        mlp = fit_mlp(X_train, y_train)
        pred_ridge = np.clip(ridge.predict(X_test), a_min=0, a_max=None)
        pred_mlp = np.clip(mlp.predict(X_test), a_min=0, a_max=None)

        pred_rows.append(
            test_df[["target_year", "node_idx", "ze2020", "libze2020", "y_true"]].assign(
                features_used=",".join(kept),
                pred_rei_created_baseline=pred_ridge,
                pred_temporal_mlp_with_rei_tensor=pred_mlp,
            )
        )
        ...
        summary_rows.append(
            {
                ...
                "temporal_mlp_with_rei_tensor_wmape": wmape(y_test, pred_mlp),
            }
        )

    return pd.concat(pred_rows, ignore_index=True), summary_rows
...
def main():
    ...
    mlp_mean = float(np.mean([r["temporal_mlp_with_rei_tensor_wmape"] for r in rows]))
    payload = {
        "tensor_path": str(TENSOR_PATH.relative_to(ROOT)),
        "feature_set": FEATURES,
        "summary_by_year": rows,
        "comparison_vs_rei_created_baseline": {
            "mean_delta": float(mlp_mean - rei_mean),
            "per_year_delta": {
                str(r["target_year"]): float(
                    r["temporal_mlp_with_rei_tensor_wmape"] - r["rei_created_baseline_wmape"]
                )
                for r in rows
            },
            "worsened_years": [
                int(r["target_year"])
                for r in rows
                if r["temporal_mlp_with_rei_tensor_wmape"] - r["rei_created_baseline_wmape"] > 1e-6
            ],
            "strictly_better_with_tolerance": bool(
                mlp_mean < rei_mean
                and all(r["temporal_mlp_with_rei_tensor_wmape"] - r["rei_created_baseline_wmape"] <= 1e-6 for r in rows)
            ),
        },
    ...
    lines = [
        "# Temporal MLP With REI Tensor v0",
        "",
