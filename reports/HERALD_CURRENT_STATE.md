...
**Source of truth:** `HERALD_PROJECT_CHARTER.md`, `HERALD_METHODOLOGICAL_DECISION_LOG.md` (DEC-001→DEC-085), `reports/canonical/HERALD_04_RESULTS_EVIDENCE_AND_CLOSED_BRANCHES.md`, `reports/canonical/HERALD_56_FR_ZE2020_PRODUCT_AND_EVIDENCE_CONTRACT.md` (product/evidence contract, DEC-081), `reports/canonical/HERALD_58_FR_ZE2020_SECTORAL_PERSISTENCE_AUDIT_SPEC.md` (engine designation DEC-084; no forecast-derived states DEC-085).
...
|-------|-----------|-------|---------|------------------|
| **Data** | **85%** | FR ZE2020, PT Municipal (278, `PT_MUNICIPALITY_CONTINENTE` — DEC-061/062 confirmed eligible, audit backfilled into the decision log 2026-06-18) all observed; NL COROP (40) all observed; PT/IT/AT harmonized Path H panel; European 27-country preflight (DEC-038) | NL gemeente proxy panel built but evidentially weak (DEC-063/065) | NL gemeente births at municipal grain (CBS open data structurally blocked — DEC-061/062); BE/other EU countries not ingested at sector×territory grain |
...
| **Economic states** | **65%** | Deterministic observed states exported for aggregate PT/IT/AT and sector FR/NL/PT (Observatory v0.1.1→v0.4); PT/KZ structural absence correctly labelled, never bare NaN. States are **descriptive of the observed trajectory** and must be labelled as such wherever displayed | — | **Forecast-derived states: CLOSED with a reasoned negative result (DEC-085), not pending.** The engine designated in DEC-084 is persistence, whose predicted change against the last observation is identically zero, so any state derived from it labels every cell STAGNATION under any threshold. No thresholds were chosen and no distribution was inspected. A direction estimator would be a new model experiment, reachable only via DEC-081 Q3 |
...
- All decisions: `reports/HERALD_METHODOLOGICAL_DECISION_LOG.md` (DEC-001→DEC-085)
