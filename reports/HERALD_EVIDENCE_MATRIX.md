...
| G-13 | The L2 top-k=5 co-growth graph shows temporal coherence (observed temporal-Jaccard metrics exceed permutation nulls) for FR and NL; stability threshold not met universally | G2 corrected controls (DEC-024c) | Bloco 2 / G2 | FR 9 sectors · NL 9 sectors · PT 8 sectors; top-k=5; COVID obs_year excluded; 199 perms N1+N2 | M1 consecutive Jaccard; M2 mean pairwise Jaccard; M3 LOYO observed only (null BLOCKED); N1 temporal (permute obs_year within territory×sector); N2 row-wise territory (within each year); BH/FDR q=0.05; seeds N1=42,N2=137 | `HERALD_G2_PREFLIGHT.md` §7 | G2_AGGREGATE_TEMPORAL_SIGNAL_SUPPORTED: FR 9/9, NL 5/9 FDR-sig (both N1+N2); PT 0/8 fails. G2_EDGE_STABILITY_NOT_SUPPORTED: M2 0.06–0.26 for all sectors/countries, far below 0.70 threshold. M3 null BLOCKED; M3 obs FR 0.287/NL 0.500/PT 0.578. Floor-p diagnostics: FR obs above all 199 nulls, null std~0.0015 (not degenerate). N2 column permutation verified degenerate (std=0, p=1.0); N2 row-wise used in gate. | 2 nulls required: N1 significant for more sectors than N2 in NL; PT fails both. Prior superseded control (commit cc48924) permuted pre-computed weights — p=0.005 invalid. | Signal statistically genuine for FR+NL but modest (FR M2 ~12% above null). Stability NOT supported. PT temporal signal not validated. No causal attribution; no individual edge claims; no pooling. | `PARTIALLY_SUPPORTED` |
