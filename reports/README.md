...
25. `canonical/HERALD_27_FR_ZE2020_RELATION_OBJECTIVE_GATE.md` — pre-training gate and
    ...
    positive edge-state, feature-family, and semantics-preserving random-target
    controls. Result: `new_relation` is the most promising local objective so far;
    temporal-only and sector-only signals are real under targeted shuffles, but
    full-feature controls still block HPC/model promotion.
