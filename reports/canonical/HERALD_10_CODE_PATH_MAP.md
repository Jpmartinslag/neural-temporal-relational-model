...
| `src/modeles/france_ze2020/train_fr_ze2020_dynamic_relation_learner.py` | HERALD_27 local relation-objective smoke: distinguishes observed typed ZE2020 x sector edges from controlled non-edges using node-pair features plus recurrence/popularity controls, unseen-pair testing, optional lagged node features, positive edge-state filtering, feature-family and pair-side ablations, combined temporal+sector shuffle, semantics-preserving random-target placebo, distance-hard/scaled-distance-hard/pair-distance-hard negatives, and target-preserving hard negatives | New 2026-07-04, local diagnostic only; new_relation is promising and temporal/sector families each carry signal, but the latest controls show surviving signal is partly source-profile driven, so no HPC launch |
