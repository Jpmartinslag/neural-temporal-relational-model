...
| `src/data/france_ze2020/build_fr_ze2020_dynamic_edge_variants.py` | HERALD_26 edge variant builder: pruned-stable, stateful, sector-only, top-k, sector-top-k, feature-compatible, feature-compatible-top-k `.csv.gz` edge tables | New 2026-07-02, see `HERALD_26_FR_ZE2020_EDGE_LEARNING_PLAN.md` -- edge construction only, falsification inputs, no recommendation, no causal claim |
...
| `hpc/france_ze2020_dynamic_graph/run_fr_ze2020_edge_variant_falsification_task.sh`, `run_fr_ze2020_edge_variant_falsification_array.sbatch`, `submit_fr_ze2020_edge_variant_falsifications_hpc.sh` | HERALD_26 dynamic edge-variant falsification array: 8 edge inputs x 5 seeds, each task writes isolated outputs under `hpc_results/fr_ze2020_dynamic_edge_variants_<RUN_ID>/<variant>/seed_<seed>/` | New 2026-07-02; falsification only, no auto-promotion and no recommendation |
