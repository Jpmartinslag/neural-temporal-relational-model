...
| `src/data/france_ze2020/build_fr_ze2020_dynamic_graph_inputs.py` | Dynamic graph input bundle: `fr_ze2020_dynamic_graph_nodes.csv`, instant `fr_ze2020_dynamic_graph_edges.csv`, expanding-memory `fr_ze2020_dynamic_graph_edges_expanding.csv.gz`, `fr_ze2020_dynamic_graph_splits.csv` | New 2026-07-01, expanded 2026-07-02, see `HERALD_25_FR_ZE2020_DYNAMIC_GRAPH_MODEL_SPEC.md` -- input construction only, no model trained, no recommendation, no causal claim |
...
| `src/modeles/france_ze2020/run_fr_ze2020_top3_entry_ranking_smoke.py` | HERALD_32 target-aligned ranking smoke: compares no-relation, formula-relation, and shuffled-relation features for `future_top3_entry_3y_label` | New 2026-07-08, local diagnostic only; promising MLP smoke, no dynamic-GNN/model/recommendation claim |
| `src/modeles/france_ze2020/run_fr_ze2020_top3_entry_falsifications.py` | HERALD_33 falsification triage for the top-3 entry objective: temporal shuffle, sector shuffle, target shuffle, and relation-feature placebo around the HERALD_32 smoke runner | New 2026-07-08, local falsification only; no model/recommendation/causal claim |
| `src/modeles/france_ze2020/run_fr_ze2020_top3_entry_lift_diagnostic.py` | HERALD_36 target-aligned relation-lift diagnostic: builds rolling prior-year lift features from relation bins and compares no-relation, base formula, lift-only, base+lift, and shuffled-lift controls for `future_top3_entry_3y_label` | New 2026-07-08, local diagnostic only; small MLP lift supports a stricter relation-layer construction test, not dynamic-GNN/model/recommendation promotion |
...
| `hpc/france_ze2020_top3_entry/*` | HERALD_34 top-3 entry HPC package: 4 scenarios x 5 seeds for the `future_top3_entry_3y_label` objective, plus descriptive G1-G5 audit | New 2026-07-08; HPC spec only until run results are collected and audited |
| `hpc/france_ze2020_top3_entry_lift/*` | HERALD_37 top-3 entry relation-lift HPC package: 4 scenarios x 5 seeds for the HERALD_36 lift hypothesis, plus descriptive G1-G6 audit | New 2026-07-10; HPC spec only until run results are collected and audited |
