...
| `src/modeles/france_ze2020/train_fr_ze2020_dynamic_relation_learner.py` | HERALD_27 local relation-objective smoke: distinguishes observed typed ZE2020 x sector edges from controlled non-edges using node-pair features plus recurrence/popularity controls and optional unseen-pair testing | New 2026-07-04, local diagnostic only; all-pair recurrence/popularity, random-target, and shuffle controls show it is not yet a validated dynamic temporal-relation model, so no HPC launch |
