...
| `src/modeles/france_ze2020/run_fr_ze2020_context_conditioned_sector_relation_gate.py`, `hpc/france_ze2020_context_sector_relation/*` | DEC-077 shared nonlinear sector-lag diagnostic conditioned on lagged ZE composition, evaluated by rolling origin and ZE-disjoint folds against no-source, pooled-linear, source/context-shuffle, and target-shuffle controls | HERALD_48/49 complete gate fail: nonlinear view loses strongly to pooled linear and source/context placebo degradation is not recurrent. This target/feature specification is closed |
| `src/modeles/france_ze2020/run_fr_ze2020_product_space_entry_density_gate.py` | DEC-078 held-out-ZE product-space density ranking of next-year RCA-specialization entry against target prevalence, current target RCA, reassigned sectors, shuffled composition/labels, and random score | HERALD_50/51 complete gate fail: density beats matched placebos but loses to current target RCA and beats both marginals in only 4/13 years. This entry-density representation is closed |
| `src/modeles/france_ze2020/run_fr_ze2020_temporal_bipartite_reconstruction_gate.py` | DEC-079 masked reconstruction over directly observed ZE2020 x A10 sector-share compositions, with exactly three hidden sectors, compositional projection, held-out ZEs, rolling time, linear/closure/ablation/shuffle controls | HERALD_52/53 complete gate fail: nonlinear integration beats Ridge, information ablations, and shuffles, but loses decisively to temporal persistence. Specification closed; not production imputation, a dynamic GNN, or recommendation logic |
| `src/modeles/france_ze2020/run_fr_ze2020_composition_transition_ranking_gate.py` | DEC-080 held-out-ZE ranking of next-year sector-share transition magnitude and direction from observed current/lagged compositions, with own-history, Ridge, information-ablation, sector/time/target-shuffle, and random controls | HERALD_54/55 complete gate fail: joint MLP uses nonlinear temporal and sector-identity information but loses to past delta and target-sector history. Specification closed; no inferred edges, dynamic-GNN claim, or recommendation logic |
