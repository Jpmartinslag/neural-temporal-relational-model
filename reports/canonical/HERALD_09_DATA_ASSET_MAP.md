...
| `data/processed/france_ze2020/fr_ze2020_dynamic_graph_edges_pruned_stable.csv.gz` | FR ZE2020, HERALD_26 edge variant | First edge-learning/pruning variant: 31,551 edges retained from the expanding edge-memory table using pre-registered stable-edge filters (`top_k_per_node=5`, `stability_score>=0.25`, `abs(signal_strength)>=0.30`, `edge_age<=5`). Local smoke did not pass the `no_edges` gate; not promoted to HPC/model claims. Registry `FR_ZE2020_DYNAMIC_EDGE_VARIANTS_V1` |
| `data/processed/france_ze2020/fr_ze2020_dynamic_graph_edges_stateful.csv.gz` | FR ZE2020, HERALD_26 edge variant | Stateful edge-memory variant: 258,460 edges reweighted as `new_relation`, `decaying_relation`, or `volatile_relation`. Local smoke improved Ridge versus `pruned_stable` but still did not pass the `no_edges` gate; not promoted to HPC/model claims. Registry `FR_ZE2020_DYNAMIC_EDGE_VARIANTS_V1` |
