...
| `data/processed/france_ze2020/fr_ze2020_sector_panel.csv`, `fr_ze2020_sector_relational_features.csv`, `fr_ze2020_relational_sector_prototype_panel.csv` | FR ZE2020, 280 zones x 9 A10 sectors, 2012-2025 | Canonical observed sector composition built directly from checksum-pinned official INSEE SIDE 2025 plus causal lag features at ZE x sector / ZE x year / sector-national grains. The nine-sector total reconciles exactly with the canonical clean panel. Source contract and the single sparse zero completed by exact partition identity are recorded in `fr_ze2020_sector_panel_source_summary.json` (DEC-076/HERALD_47). The observed panel remains contemporaneous, not model-ready by itself. Registry: `PANEL_FR_ZE2020_SECTOR_COMPOSITION`, `PANEL_FR_ZE2020_SECTOR_RELATIONAL_FEATURES`, `PANEL_FR_ZE2020_RELATIONAL_SECTOR_PROTOTYPE`. |
| `data/processed/france_ze2020/fr_ze2020_exploratory_relation_signals.csv`, `fr_ze2020_exploratory_relation_examples.csv` | FR ZE2020, 4 relation families (6,215 + 20 rows) | Retrospective interpretation layer only. Its recurrence and stability fields summarize the complete observation window, so it is forbidden as an as-of-time model input. See `HERALD_20` and the correction in `HERALD_38`; registry `FR_ZE2020_EXPLORATORY_RELATION_SIGNALS_V1` |
| `data/processed/france_ze2020/fr_ze2020_temporal_relation_signals.csv.gz` | FR ZE2020 x sector, 2017-2025 | Leakage-safe as-of-time relation snapshots: 226,980 rows across ZE similarity, cross-ZE same-sector, and intra-ZE sector relations. Recurrence and stability at decision year `t` use only years `<=t`. This is the only current relation-signal input authorized for model construction. See `HERALD_38`; registry `FR_ZE2020_TEMPORAL_RELATION_SIGNALS_V1` |
| `data/processed/france_ze2020/fr_ze2020_sector_ranking_panel.csv` | FR ZE2020, 280 zones x 9 A10 sectors x 2012-2025 | Canonical ZE-sector ranking panel regenerated from temporal snapshots. Target columns are evaluation-only; a horizon-`h` label is trainable at evaluation year `t` only when `decision_year + h <= t`. See `HERALD_24` and `HERALD_38`; registry `FR_ZE2020_SECTOR_RANKING_PANEL_V1` |
| `data/processed/france_ze2020/fr_ze2020_dynamic_graph_nodes.csv`, `fr_ze2020_dynamic_graph_edges.csv`, `fr_ze2020_dynamic_graph_edges_expanding.csv.gz`, `fr_ze2020_dynamic_graph_splits.csv` | FR ZE2020, 280 zones x 9 A10 sectors x 2012-2025 snapshots | Corrected dynamic graph input bundle: 35,280 node-year rows, 226,980 instant typed edges, 661,613 expanding-memory rows, and 14 split rows. Built from the ranking panel and temporal relation snapshots only. Prior bundles/results are invalid for claims pending rerun. Registry `FR_ZE2020_DYNAMIC_GRAPH_INPUTS_V1` |
| `data/processed/france_ze2020/fr_ze2020_dynamic_graph_edges_pruned_stable.csv.gz`, `fr_ze2020_dynamic_graph_edges_stateful*.csv.gz`, `fr_ze2020_dynamic_graph_edges_feature_compatible*.csv.gz`, `fr_ze2020_dynamic_graph_edges_learned*.csv.gz`, `fr_ze2020_dynamic_graph_edges_precision*.csv.gz` | FR ZE2020, HERALD_26 edge variants | Regenerated from the corrected 661,613-row expanding-memory graph. The 2026-07-02 HPC findings used the superseded edge bundle and are invalid for claims until rerun. These remain falsification inputs, not model promotion. Registry `FR_ZE2020_DYNAMIC_EDGE_VARIANTS_V1` |
| `data/processed/france_ze2020/fr_ze2020_commuting_edges.csv.gz`, `fr_ze2020_commuting_edge_summary.json` | FR ZE2020, official commuting relation source | 86,568 directed edge-snapshot rows from official INSEE 2012/2017/2023 commune residence-to-workplace flows, reconciled with official COG events to the canonical 280-zone scope. Observation-time and strict ex-ante clocks are separate. Edge table is regenerable/untracked; summary is tracked. Not yet a model input. See HERALD_44 / DEC-073 |
| `data/processed/france_ze2020/fr_ze2020_commuting_strict_ex_ante_edges.csv.gz`, `fr_ze2020_commuting_strict_ex_ante_summary.json` | FR ZE2020, strict ex-ante commuting candidate | 276,790 directed cross-ZE edge-year rows for 2016--2025. Release-aware assignment uses the 2012 snapshot through decision year 2020 and the 2017 snapshot from 2021; the 2023 release is never used in the current panel. Candidate only pending matched-placebo gate. See HERALD_44 / DEC-073 |
