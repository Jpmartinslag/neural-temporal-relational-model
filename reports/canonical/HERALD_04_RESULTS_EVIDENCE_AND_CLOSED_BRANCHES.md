...
| Causal per-country baseline (Path M, FR/NL/BE/PT, distinct from Path H above) | Phase 4E-B clean causal baseline per country: FR 0.1031, NL 0.1017, BE 0.1488, PT 0.2286 (no single feature policy dominates across all four) | **VALIDATED** as the per-country reference; not a cross-country pooled claim | DEC-002, `HERALD_PHASE4E_B_RESULTS_AUDIT.md` | Yes, per-country only — never pooled (targets are heterogeneous, see DEC-003 row below) |
...
| SharedRelationEncoder detects real sector relations | Synthetic: unseen-pair AUC=0.690 (strong). Real: sign concordance 0.438 zero-shot, 0.500 honest fine-tuned figure (DEC-059, excludes a single inflated low-evidence FR label that made DEC-058's 0.667 look better than it was), 4/7 controls within 0.05 of that figure, 0 abstentions ever recorded | `REAL_WEAK_LABEL_TUNING_PARTIAL` | DEC-055/056/058/059 | Synthetic result: yes. Real-data result: only as "partial, not final," never as a headline claim |
