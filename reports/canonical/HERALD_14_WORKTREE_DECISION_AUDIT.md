...
| `src/data/european_panel/build_nl_gemeente_phase7_panel.py`, `preflight_enterprise_birth_candidates.py`, `src/data/ingest_eurostat_enterprise_birth_panel.py`, `src/modeles/herald_fold_controls.py`, `src/data/european_panel/build_fr_nuts3_sector_panel.py` (+ matching test `test_build_fr_nuts3_sector_panel.py`) | untracked | small | DEC-063/065 (NL gemeente), DEC-038 (Eurostat coverage) | CODE + TEST | ACTIVE (NL gemeente ingestion, Eurostat ingestion) | MEDIUM | `COMMIT_AFTER_TESTS` | Source code behind already-cited DEC-063/065/038 results is untracked; `herald_fold_controls.py` usage not yet traced to a specific DEC — verify importers before bundling | `pytest tests/test_build_fr_nuts3_sector_panel.py -q`; check for tests covering `build_nl_gemeente_phase7_panel.py`/`preflight_enterprise_birth_candidates.py`/`ingest_eurostat_enterprise_birth_panel.py`/`herald_fold_controls.py` before commit |
...
| `RESOLVED_COMMITTED_CODE_TESTS` | 3 (G1-L1/observable builders + tests, 2026-06-19; dual-graph builders + tests, 2026-06-19; ARDECO bundle, 2026-06-19 — all with outputs intentionally left untracked) |
| `COMMIT_AFTER_TESTS` | 4 (hpc/phase4 scripts, real weak-label outputs, eurostat decoder, NL gemeente/Eurostat-ingestion/build_fr_nuts3_sector_panel bundle) — top-level `scripts/` symlinks excluded: they embed an absolute local path (`/home/jpdark/...`) and would break on any other clone |
