...
| `train_herald_v6.py`, `train_herald_v7.py`, `train_herald_semi_v2.py`, `train_herald_regime_experiment.py` (modified) | pre-Q7 HISTORICAL_EXPERIMENT scripts per canonical #10 | modified (tracked) | 15-119 line diffs each | predates DEC log; superseded by Q7 | CODE | HISTORICAL (superseded) | MEDIUM | `HISTORICAL_ARCHITECTURE_ATTEMPT_HOLD` | Human clarification on 2026-06-24: these modified files are intentional architecture-improvement attempts kept to show how the architecture evolved, not accidental scope creep. They still depend on the invalid legacy panel path and remain `PENDING_REAUDIT`/not current; do not use for claims or current training until explicitly reaudited | Preserve as architecture-history evidence; do not move/delete/commit as current path in the data-pipeline cleanup. Any future use needs a separate reauditing task against `fr_ze2020_model_ready_panel.csv` |
...
| `HISTORICAL_ARCHITECTURE_ATTEMPT_HOLD` | 1 (train_herald_v6/v7/semi/regime modifications) — **intent clarified 2026-06-24; preserved as architecture-history evidence, not current training path** |
| `HUMAN_REVIEW_REQUIRED` | 3 (~48 untested `hpc/phase4` scripts; Eurostat DEC-038 ingestion/preflight scripts with no test; `herald_fold_controls.py` + stray `process_bd_hgnace_r.py`) — **none touched, all still need a human call** |
...
3. **`train_herald_v6.py`/`v7.py`/`semi_v2.py`/`regime_experiment.py` modifications
   (`HISTORICAL_ARCHITECTURE_ATTEMPT_HOLD`, updated 2026-06-24).** Real new functionality
   (e.g. V6's `fit_zone_mask` parameter) was added to scripts otherwise superseded by Q7.
   Human decision now clarified that these are intentional architecture-history attempts,
   useful for explaining where the architecture went. They still do **not** become current
   training code and do **not** support current claims until a separate reauditing task
   ports/validates any chosen idea against `fr_ze2020_model_ready_panel.csv`.
