...
| `hpc/phase7_sector_precedence/configs/full_run.json` | single config file | untracked | tiny | DEC-033/034 | HPC config | **VALID** (the exact config behind the cited run) | MEDIUM | `COMMIT_AFTER_TESTS` | Tiny, directly documents the n_permutations=999/n_bootstraps=500 parameters already cited; low-risk to commit alongside the sector_precedence_results fix above | Commit together with the sector_precedence_results group |
...
| Bibliography additions | `reports/bibliography/HERALD_REFERENCES_MASTER.md` | modified (tracked) | +97/-3 lines, pure addition | DEC-057 (Axis 15, weak supervision references: Snorkel, Group DRO) | DOC | VALID (reference material) | LOW | `COMMIT_NOW_DOC_ONLY` | Purely additive reference entries tied to an already-cited DEC (DEC-057); no removal, no science altered, safe to commit as documentation | `git add reports/bibliography/HERALD_REFERENCES_MASTER.md` and commit separately (not bundled with this task per its own rules) |
| `src/data/european_panel/build_nl_gemeente_phase7_panel.py`, `preflight_enterprise_birth_candidates.py`, `src/data/ingest_eurostat_enterprise_birth_panel.py`, `src/modeles/herald_fold_controls.py`, `src/modeles/run_ardeco_ridge_fr.py` (+ matching tests `test_ardeco_fr_extension.py`, `test_ardeco_ridge_fr.py`, `test_build_fr_nuts3_sector_panel.py`) | untracked, mostly bundled with their tests | small | DEC-063/065 (NL gemeente), DEC-038 (Eurostat coverage), ARDECO closed exploration | CODE + TEST | Mixed: ACTIVE (NL gemeente, Eurostat ingestion) / CLOSED (ARDECO) | MEDIUM | `COMMIT_AFTER_TESTS` | Same traceability-gap pattern: source code behind already-cited DEC-063/065/038 results is untracked. ARDECO scripts are closed-exploration but still deserve a code+test commit for audit trail | `pytest tests/test_ardeco_fr_extension.py tests/test_ardeco_ridge_fr.py tests/test_build_fr_nuts3_sector_panel.py -q`, then commit each bundle |
...
| `COMMIT_NOW_DOC_ONLY` | 1 (bibliography) |
