# HERALD Codex Memory
**LEIA PRIMEIRO.** Updated 2026-06-13 (DEC-040: Phase 9 full benchmark infra PASS; DEC-041: G3 convergence probe G3_NOT_CONFIRMED/FULL_HPC_AUTHORIZED).
...
- **DEC-041 — G3 block-masking convergence probe (G3_NOT_CONFIRMED):** Targeted probe for linear/seed=123 at 200/300/500 epochs. Block masking G3 failure is STRUCTURAL and SEED-SPECIFIC (seeds 42+456 pass block masking; 3-seed aggregate G3 PASSES for linear). G3_NOT_CONFIRMED for seed=123; does not block HPC. G4 FAIL confirmed at all epoch budgets (mean cal90=0.260). Determinism CONFIRMED (all metric differences are wall-clock train_s only). **FULL_HPC_AUTHORIZED** (epoch budget frozen at 500; explicit sbatch authorisation required). Conformal calibration spec documented for post-HPC task. Script: `src/modeles/synthetic/run_convergence_probe.py`.
- **Immediate next step (Phase 9):** Submit HPC array (FULL_HPC_AUTHORIZED — requires explicit sbatch authorisation): `sbatch hpc/phase9_synthetic_generalization/run_phase9.slurm`. Epoch budget: 500. Output: `data/processed/synthetic_benchmark/full/`. Post-run: evaluate gates. OR: Phase 7 extension (download national source for ES/IT/FI mapping).
