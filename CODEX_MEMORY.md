# HERALD Codex Memory
**LEIA PRIMEIRO.** Updated 2026-06-15 (DEC-051 IMPLEMENTATION_COMPLETE — experimento pendente execução local. Phase 15: clamped NLL + Huber + GraphAuxHeads independentes + NT1-NT6 audit. 38 testes PASS. 300 épocas NÃO executadas — autorização explícita necessária. DEC-050: TEMPORAL_MASKED@75 MAE=0.2327 bate ffill 9.4% em novel_lag2 zero-shot. GRAPH_MULTITASK instável — colapso log_sigma→-∞).
...
- **DEC-051 — Stable Objective Audit (IMPLEMENTATION_COMPLETE; experimento pendente):** Objectivo: confirmar se ganho few-shot é real (NT1-NT6) + estabilizar loss (R1=NLL clamped, R2=Huber, R3=MSE) + cabeças grafais independentes (sign_logit, lag_logit como nn.Parameter separados). 5 variantes × 3 budgets (30/75/150) × 5 seeds × 2 cenários × 2 máscaras. Top-2 seleccionadas por val_loss (NOT test). Gates V1-V10 congelados. 300 épocas NÃO executadas — autorização explícita necessária. 38 testes PASS. Package: `src/modeles/synthetic/phase15_stable_objective/` (7 ficheiros). Relatório: `reports/HERALD_DEC051_STABLE_OBJECTIVE_AUDIT.md`.
