# HERALD Codex Memory
**LEIA PRIMEIRO.** Updated 2026-06-16 (DEC-056 COMPLETE — 6/10 PASS. REAL_SHARED_RELATION_PARTIAL. Zero-shot encoder: presence scores ~0.067 (abaixo threshold 0.55). R3 PASS: stability FR=0.702/NL=0.549/PT=0.614. R4 PASS: Phase 7 sign concordance=0.562 (16 edges). R2/R5/R6 FAIL: checkpoint necessário para candidate detection. 3.4s CPU.).
...
- **DEC-056 — Real Shared Relation Encoder Validation (COMPLETE; 6/10 PASS; REAL_SHARED_RELATION_PARTIAL):** Valida SharedRelationEncoder (zero-shot, sem checkpoint DEC-055) em dados reais FR/NL/PT. Protocolos: P0 (zero-shot log1p norm), P1 (leave-one-country-out z-score calib), 3 controlos de permutação. 1096 registos pair-window. R1 PASS (sem leakage/NaN/Inf; PT KZ excluído). R2 FAIL: controlos indistinguíveis do real (presence ~0.067 = prior inicialização; untrained encoder). R3 PASS: stability Spearman FR=0.702, NL=0.549, PT=0.614 (>0.30 em 3 países). R4 PASS: Phase 7 sign concordance=0.562 (16 edges comparadas). R5/R6 FAIL: nenhuma associação acima threshold 0.55 em modo zero-shot. R7 PASS: períodos COVID reportados separadamente. R9/R10 PASS: sem linguagem causal, schema válido. Decisão: FEATURES INFORMATIVAS (estabilidade + concordância de sinal) mas checkpoint necessário para candidate detection. 56/56 testes PASS. Elapsed: 3.4s CPU. Package: `src/modeles/real_world/`, gates: `src/modeles/synthetic/phase16_decoupled/gates_dec056.py`. Results: `data/processed/real_shared_relations/`. Report: `reports/HERALD_DEC056_REAL_SHARED_RELATION_AUDIT.md`.
