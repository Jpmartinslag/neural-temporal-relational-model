# HERALD Codex Memory
**LEIA PRIMEIRO.** Updated 2026-06-15 (DEC-050 TEMPORAL_MASKED_CONFIRMED; GRAPH_MULTITASK_UNSTABLE: 3 bugs corrigidos em pretrain_runner.py (A/B/C). Resultado chave: TEMPORAL_MASKED@75 MAE=0.2327 BATE ffill=0.2568 em novel_lag2 (9.4% melhoria). GRAPH_MASKED_MULTITASK instável a 50 datasets — colapso de variância (val_loss -3→-32000). Few-shot A1: 78-80% redução MAE em todas variantes. 4/10 gates PASS. 300-epoch trigger disparado mas NÃO executado sem autorização. 30 testes PASS. Próximo: decidir se TEMPORAL_MASKED 300 épocas ou nova arquitectura para GRAPH_MASKED_MULTITASK (gradient clipping)).
