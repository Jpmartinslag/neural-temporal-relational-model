# HERALD Codex Memory
**LEIA PRIMEIRO.** Updated 2026-06-15 (DEC-053 IMPLEMENTADO — arquitectura desacoplada Phase 16: GraphRelationHead + GatedGraphModel + UtilityGate. 41/41 testes PASS. Gates D1-D10 congelados. Pronto para execução local com backbone Phase 15).
...
- **DEC-053 — Decoupled Graph Audit (IMPLEMENTADO; pronto para execução):** Causa raiz do no-graph ganhar: adj_s simétrico adiciona reversos falsos; cenários novel frac_nonlinear=0.85-0.90 onde temporal features são suficientes. Arquitectura: A=GraphRelationHead (presença/sinal/lag dirigidos); B=TemporalDecoder (backbone congelado, adj=0); C=GraphMessageExpert (residual dirigido clamped); D=UtilityGate (fechado na init bias=-5, sem alvo). 3 modos: ANALYTIC_GRAPH_ONLY, TEMPORAL_RECONSTRUCTION, GATED_GRAPH_ASSIST. Loss desacoplada com λ congelados. Fixtures F1-F6. Gates D1-D10 congelados. 41/41 testes PASS. Execução: `python -m src.modeles.synthetic.phase16_decoupled.run_dec053 --backbone_path <checkpoint.pt> --n_sectors 5`. Package: `src/modeles/synthetic/phase16_decoupled/` (7 ficheiros). Relatório: `reports/HERALD_DEC053_DECOUPLED_GRAPH_AUDIT.md`.
